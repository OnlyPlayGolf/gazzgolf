...
const CATEGORIES: Record<string, CategoryConfig> = {
  ...
  'strokes-gained': {
    ...
    getStats: (stats) => [
      ...
      { label: 'Other', value: formatSG(stats.strokesGained.other), isPositive: (stats.strokesGained.other ?? 0) >= 0 },
      { label: 'Scoring', value: formatSG(stats.strokesGained.scoring), isPositive: (stats.strokesGained.scoring ?? 0) >= 0 },
    ],
  ...
  'sg-total': {
    title: 'SG: Total',
    icon: TrendingUp,
    description: 'Your complete strokes gained breakdown',
    whatItMeasures: 'Total Strokes Gained is the sum of all categories: Off the Tee, Approach, Short Game, Putting, Other, and Scoring. It shows your overall performance versus a baseline.',
    whyItMatters: 'This single number tells you if you\'re playing better or worse than the baseline. Tour pros average +2 to +4 total SG per round.',
    tips: [
      'Track your total SG trend over time',
      'Identify which category contributes most',
      'A consistent +1 SG per round drops your handicap significantly'
    ],
    getStats: (stats) => [
      { label: 'Total SG', value: formatSG(stats.strokesGained.total), isPositive: (stats.strokesGained.total ?? 0) >= 0 },
      { label: 'Off the Tee', value: formatSG(stats.strokesGained.offTheTee), isPositive: (stats.strokesGained.offTheTee ?? 0) >= 0 },
      { label: 'Approach', value: formatSG(stats.strokesGained.approach), isPositive: (stats.strokesGained.approach ?? 0) >= 0 },
      { label: 'Short Game', value: formatSG(stats.strokesGained.shortGame), isPositive: (stats.strokesGained.shortGame ?? 0) >= 0 },
      { label: 'Putting', value: formatSG(stats.strokesGained.putting), isPositive: (stats.strokesGained.putting ?? 0) >= 0 },
  ...
  'sg-off-the-tee': {
    ...
    icon: Target,
    description: 'Driving performance analysis',
    whatItMeasures: 'Measures your performance with tee shots on par 4s and 5s compared to baseline. Accounts for both distance and accuracy.',
    whyItMatters: 'A good drive sets up the hole. Tour pros average +0.5 to +1.0 SG off the tee. Finding fairways leads to easier approaches.',
    tips: [
      'Prioritize fairways over maximum distance',
      'Develop a reliable shot shape you can count on',
      'Know which holes require driver vs. fairway wood',
      'Practice hitting targets, not just distance'
    ],
    getStats: (stats) => [
      { label: 'SG Off the Tee', value: formatSG(stats.strokesGained.offTheTee), isPositive: (stats.strokesGained.offTheTee ?? 0) >= 0 },
      { label: 'Fairways Hit', value: formatPercentage(stats.accuracy.fairwaysHit) },
  ...
  'sg-approach': {
    title: 'SG: Approach',
    icon: Target,
    description: 'Iron play into greens',
    whatItMeasures: 'Measures your iron shots from 40+ meters to the green. Considers proximity to hole and avoiding trouble.',
    whyItMatters: 'Approach play is often the biggest differentiator between handicap levels. Tour pros average +0.8 to +1.5 SG approach.',
    tips: [
      'Know your exact yardages for each club',
      'Miss on the correct side of the green',
      'Aim for the center of the green more often',
  ...
  'sg-short-game': {
    ...
    whatItMeasures: 'Measures chips, pitches, and bunker shots from within 40 meters (excluding putting). Shows how well you recover.',
    whyItMatters: 'Great short game saves par and prevents big numbers. Tour pros scramble 60%+ while amateurs average 30%.',
    tips: [
      'Master one go-to chip shot first',
      'Practice from different lies and stances',
      'Get the ball on the green and rolling quickly',
  ...
  'sg-putting': {
    title: 'SG: Putting',
    icon: Target,
    description: 'Performance on the greens',
    whatItMeasures: 'Measures your putting compared to baseline from various distances. Accounts for putt length and make percentage.',
    whyItMatters: 'Putting is about 40% of all strokes. Tour pros average +0.3 to +0.8 SG putting. One-putt percentage matters most.',
    tips: [
      'Focus on distance control for lag putts',
  ...
  'sg-other': {
    title: 'SG: Other',
    icon: TrendingUp,
    description: 'Miscellaneous shots analysis',
    whatItMeasures: 'Captures strokes gained/lost from recovery shots, penalty situations, and other non-standard situations not fitting main categories.',
    whyItMatters: 'Big losses here often indicate course management issues or trouble with recovery shots. Minimizing mistakes is key.',
    tips: [
      'Take your medicine - play safe from trouble',
      'Practice punch shots and recovery options',
      'Know when to take penalty relief',
      'Avoid compound errors (trouble into more trouble)'
    ],
    getStats: (stats) => [
      { label: 'SG Other', value: formatSG(stats.strokesGained.other), isPositive: (stats.strokesGained.other ?? 0) >= 0 },
      { label: 'Total SG', value: formatSG(stats.strokesGained.total), isPositive: (stats.strokesGained.total ?? 0) >= 0 },
    ],
    getRelevantDrills: () => []
  },
  'sg-scoring': {
    title: 'SG: Scoring',
    icon: TrendingUp,
    description: 'Scoring efficiency analysis',
    whatItMeasures: 'Measures how effectively you convert opportunities into birdies and avoid bogeys. Tracks your decision-making impact on score.',
    whyItMatters: 'Good course management and clutch performance directly impact your final score. It\'s about making smart decisions.',
    tips: [
      'Play to your strengths on birdie holes',
      'Minimize doubles and triples',
      'Know when to be aggressive vs. conservative',
      'Par is always a good score when in trouble'
    ],
    getStats: (stats) => [
      { label: 'SG Scoring', value: formatSG(stats.strokesGained.scoring), isPositive: (stats.strokesGained.scoring ?? 0) >= 0 },
      { label: 'Scoring Average', value: stats.scoring.scoringAverage !== null 
        ? (stats.scoring.scoringAverage > 0 ? '+' : '') + stats.scoring.scoringAverage.toFixed(1) 
        : '-' },
      { label: 'Best Round', value: stats.scoring.bestRound?.toString() ?? '-' },
