-- Add level_number column to level_progress table
ALTER TABLE public.level_progress 
ADD COLUMN IF NOT EXISTS level_number integer;

-- Update existing records with level numbers based on level_id
-- This will be a one-time update, future inserts will include the level_number

-- Drop and recreate the friends level leaderboard function with category filtering
...
AS $function$
  ...
  qualified_highest AS (
    SELECT 
      user_id,
      max_level_in_category as highest_level,
      category
    FROM category_counts
    WHERE count_in_category >= 10
    ORDER BY max_level_in_category DESC
    LIMIT 1
  ),
  user_stats AS (
    SELECT 
    ...
    LEFT JOIN qualified_highest qh ON qh.user_id = up.user_id
...
-- Drop and recreate the favourite groups level leaderboard function with category filtering
DROP FUNCTION IF EXISTS public.favourite_groups_level_leaderboard();
